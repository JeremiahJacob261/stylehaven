...
const defaultProps: SupremeReceiptProps = {
  ...
  SHIPPING_ADDRESS_2: "123 Hype Street",
  SHIPPING_ADDRESS_3: "New York, NY 10013",
  SHIPPING_ADDRESS_4: "United States",
  BILLING_ADDRESS_1: "John Doe",
  BILLING_ADDRESS_2: "123 Hype Street",
  BILLING_ADDRESS_3: "New York, NY 10013",
  BILLING_ADDRESS_4: "United States",
  PAYMENT_METHOD: "Mastercard",
  CARD_ENDING: "5678"
};

interface SupremeReceiptPageProps {
  receiptData?: SupremeReceiptProps;
  receiptId?: string;
}

const SupremeReceiptPage: React.FC<SupremeReceiptPageProps> = ({ 
  receiptData: serverReceiptData, 
  receiptId 
}) => {
  const [receiptData, setReceiptData] = useState<SupremeReceiptProps>(
  ...
  return (
    <>
      <Head>
        <title>{`Supreme - Order ${receiptData.ORDER_NUMBER}`}</title>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  if (id && typeof id === 'string') {
    try {
      ...
      if (receipt && receipt.receipt_type === 'supreme') {
        return {
          props: {
