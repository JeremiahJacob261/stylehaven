...
export function TrapstarReceiptForm({ data, onInputChange }: TrapstarReceiptFormProps) {
  return (
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input 
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
            placeholder="/trapstar/trapstar_files/product-image.jpg"
          />
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 gap-3">
          {[1, 2, 3, 4].map((i) => (
            <div key={i}>
              ...
              <input 
                type="text" 
                value={data[`SHIPPING_ADDRESS_${i}` as keyof TrapstarReceiptData]} 
                onChange={(e) => onInputChange(`SHIPPING_ADDRESS_${i}` as keyof TrapstarReceiptData, e.target.value)} 
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                placeholder={i === 1 ? "John Doe" : i === 2 ? "123 Main Street" : i === 3 ? "London W1K 5AB" : "United Kingdom"}
              />
            </div>
          ))}
        </div>
