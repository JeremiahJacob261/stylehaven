...
export function YzyGapReceiptForm({ data, onInputChange }: YzyGapReceiptFormProps) {
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <input type="text" value={data.PRODUCT_IMAGE} onChange={e => onInputChange('PRODUCT_IMAGE', e.target.value)} placeholder="/yzygap/yzygap_files/product-image.jpg" className="w-full px-3 py-2 border rounded-lg" />
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3">
          {[1,2,3,4].map(i => (
            <input key={i} type="text" value={data[`SHIPPING_ADDRESS_${i}` as keyof YzyGapReceiptData] as string} onChange={e => onInputChange(`SHIPPING_ADDRESS_${i}` as keyof YzyGapReceiptData, e.target.value)} placeholder={`Address Line ${i}`} className="w-full px-3 py-2 border rounded-lg" />
          ))}
        </div>
      </div>
      {/* Payment Information */}
